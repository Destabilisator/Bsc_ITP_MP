...
if __name__ == "__main__":
    ...
    for J in range(len(X)):
        ...
        if LINEAR_FIT:
            ...
            subfig1.plot(figData_ED_X, figData_ED_Y, lw = 0, ls = "solid", markersize = 16, marker = "h", color = "blue", label = "ED")
            subfig1.errorbar(figData_QT_X, figData_QT_Y, yerr = figData_QT_YErr, lw = 0, ls = "dashed", markersize = 8, elinewidth = 3, fmt = "s", color = "red", label = "QT")
            ...
            fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "linear" + ".png")
            # fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "linear" + ".pdf")
            plt.close(fig1)

        if ONE_OVER_SQRTN_FIT:
            figData_QT_X = np.asarray(figData_QT_X)
            figData_QT_X = 1 / np.sqrt(figData_QT_X)
            figData_ED_X = np.asarray(figData_ED_X)
            figData_ED_X = 1 / np.sqrt(figData_ED_X)
            ...
            fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "sqrt" + ".png")
            # fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "sqrt" + ".pdf")
            plt.close(fig1)

        if ONE_OVER_N_FIT:
            figData_QT_X = np.asarray(figData_QT_X)
            figData_QT_X = 1 / figData_QT_X
            figData_ED_X = np.asarray(figData_ED_X)
            figData_ED_X = 1 / figData_ED_X
            ...
            fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "N" + ".png")
            # fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "N" + ".pdf")
            plt.close(fig1)

        if ONE_OVER_N2_FIT:
            figData_QT_X = np.asarray(figData_QT_X)
            figData_QT_X = 1 / figData_QT_X**2
            figData_ED_X = np.asarray(figData_ED_X)
            figData_ED_X = 1 / figData_ED_X**2
            ...
            fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "N2" + ".png")
            # fig1.savefig("results/" + "SGExtrapFit/J_" + J_val + "N2" + ".pdf")
            plt.close(fig1)
