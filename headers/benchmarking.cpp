        ...
        for (int N = N_start; N <= N_end; N += 2) {
            ...
            for (double stepsize : {0.1, 0.01}) {
                ...
                for (int SAMPLES : {1, 2, 3}) {
                    std::cout << "\nBENCHMARKING: N = " << N << " (" << SIZE << "), stepsize = " << stepsize << ", SAMPLES = " << SAMPLES;
                    // QT
                    ...
                    save_bench_val("QT_SG_step_" + std::to_string(stepsize) + "_SAMPLES_" + std::to_string(SAMPLES) + "_cores_" + std::to_string(cores) + ".txt", std::to_string(N) + "\t" + std::to_string(elapsed_seconds_QT.count()));
                }
                // ED with fit
                ...
                save_bench_val("ED_MJ_step_" + std::to_string(stepsize) + "_cores_" + std::to_string(cores) + ".txt", std::to_string(N) + "\t" + std::to_string(elapsed_seconds_ED_MJ.count()));
            }
            // ED from EV
            std::cout << "\nBENCHMARKING: N = " << N << " (" << SIZE << ")";
            ...
            save_bench_val("ED_SG_cores_" + std::to_string(cores) + ".txt", std::to_string(N) + "\t" + std::to_string(elapsed_seconds_ED_SG.count()));
        }
    }
