...
// [executable] $N $START $END $COUNT $h $CORES $noX $SILENT
// [executable] 3D $N $J_START $J_END $J_COUNT $T_START $T_END $T_COUNT $h $CORES $noX $SILENT
void validateInput(int &argc, char *argv[], const unsigned int &cpu_cnt, int &N, int &SIZE,
                   double &J_START, double &J_END, int &J_COUNT,
                   double &T_START, double &T_END, int &T_COUNT,
                   double &h, double &h_START, double &h_END, int &h_COUNT,
